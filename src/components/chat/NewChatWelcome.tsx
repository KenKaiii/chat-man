...
interface NewChatWelcomeProps {
  ...
  onSubmit: (files?: FileAttachment[], mode?: 'general' | 'rag' | 'spark' | 'voice') => void;
  onStop?: () => void;
  disabled?: boolean;
...
const CAPABILITIES = [
  ...
  "I assist with research and brainstorming privately",
  "I review contracts and confidential documents securely",
  "Your data never touches the cloud - ever"
];
...
export function NewChatWelcome({ inputValue, onInputChange, onSubmit, onStop, disabled, isGenerating, isPlanMode, onTogglePlanMode, availableCommands = [] }: NewChatWelcomeProps) {
  ...
  useEffect(() => {
    fetch('/api/user-config')
      ...
      .catch(err => {
        // Silently fail - user config is optional
        console.debug('User config not available:', err.message);
      });
  }, []);
        ...
        <div className="flex flex-col gap-1 justify-center items-center mb-8">
          <div className="flex flex-row justify-center gap-3 w-fit px-5">
            <div className="text-[40px] font-semibold line-clamp-1 text-gradient">
              {userName ? `Hi, ${userName}. I'm Chat Man` : "Hi. I'm Chat Man"}
            </div>
          </div>
